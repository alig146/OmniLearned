...
class PET2(nn.Module):
    def __init__(
        ...
        self.track_task_names = {"tautrack_class"} # Tasks in this set are predicted per track token (B, T, C).
        self.track_aux_heads = nn.ModuleDict()
        event_aux_tasks = [ task for task in self.aux_tasks if task["name"] not in self.track_task_names ]
        track_aux_tasks = [ task for task in self.aux_tasks if task["name"] in self.track_task_names ]
        ...
        if use_classifier:
            ...
            for task in track_aux_tasks:
                self.track_aux_heads[task["name"]] = nn.Linear( base_dim, task["num_classes"] )
